...
/**
 * Translate view — combines translator + phrasebook.
 */
export function renderTranslate(container) {
    ...
    <section class="view translate-view">
      ...
      <div class="translator-controls">
        ...
        <div class="translator-input-row">
          ...
          <button id="translator-play" class="btn btn--icon" aria-label="Play audio" disabled>&#9654;</button>
        </div>
      </div>
      <div id="translator-results" class="translator-results" aria-live="polite"></div>

      <details class="translate-phrases" open>
  ...
  playBtn.addEventListener('click', () => {
    if (lastSentenceWords.length > 0) playSentence(lastSentenceWords);
    else if (lastSyllables.length) playWord(lastSyllables);
  });
  ...
  function doTranslate() {
    ...
    if (direction === 'sentence') doSentence(q);
    else if (direction === 'en-sol') doEnglishToSolresol(q);
    else doSolresolToEnglish(q);
  }
  ...
  function doEnglishToSolresol(q) {
    ...
    matches.slice(0, 30).forEach(entry => {
      ...
      const wr = createWordRenderer(word, {
        size: 'sm', showSheet: false, showDefinition: true, showReverse: word.length >= 2,
        reactive: false, notations: new Set(['colors', 'solfege']),
      });
  ...
  function doSolresolToEnglish(q) {
    ...
    const wr = createWordRenderer(word, {
      size: 'lg', showSheet: true, showDefinition: true, showReverse: word.length >= 2,
      reactive: true, notations: new Set(['colors', 'solfege', 'numbers', 'binary']),
    });
  ...
  for (const cat of PHRASE_CATEGORIES) {
    ...
    for (const phrase of cat.phrases) {
      ...
      const allSyls = phrase.solresol.split(' ').flatMap(w => parseWord(w));
      ...
      const phrasePlayBtn = document.createElement('button');
      phrasePlayBtn.className = 'btn btn--sm btn--icon';
      phrasePlayBtn.innerHTML = '&#9654;';
      phrasePlayBtn.addEventListener('click', () => playWord(allSyls));
      solEl.append(wordText, blocks, phrasePlayBtn);
